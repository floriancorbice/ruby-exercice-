...
print "Combien y a t'il de journalistes dans cet array ? " 
puts journalistes.count

print "Combien d'handle contiennent un numéro ? "
num = journalistes.select {|item| item =~ /[0-9]/}
puts num.count

print "Combien d'handle contiennent les 4 lettres du prénom aude? "
aude = journalistes.select {|item| item.downcase.include?("aude")}
puts aude.count

print "Combien contiennent une majuscule en début de nom ? "
début = journalistes.select {|item| item[1..1] =~ /[A-Z^]/}
puts début.count


print "Combien contiennent une majuscule ? "
maj = journalistes.select {|item| item =~ /[A-Z]/}
puts maj.count

Dash = journalistes.select   { |z| z =~ /[_]/}
print "Combien de _ dans tous les pseudos confondus ? "
puts Dash.count

puts "Trie la liste de handle par ordre alphabétique!"
alphabet = journalistes.map 
